"""Calculate the 3-30-300 rule for urban trees in a district."""
...
if __name__ == "__main__":
    ...
    projection = params["EPSG"][f"{municipality}"]
    district_numbers = params["district_numbers"][f"{municipality}"]

    print(f"Running for municipality: {municipality}"
          f" with district numbers: {district_numbers}"
          f" and projection: {projection}")

    # catalog (file paths)
    parquet_dir = catalog["parquet_dir"]["file_path"]
    geojson_dir = catalog["geojson_dir"]["file_path"]
    reporting_dir = catalog["rule_3_30_300_dir"]["file_path"]
